...
function mise_a_jour(table) {
    for (let i = 0; i < table.length; i++) {
        if (table[i].timestamp >= initialeDate && table[i].timestamp <= finalDate) {
            console.log("Bonjours chers(es) clients(es) ", table[i].id, "Il vous ait accordez un Bonus de 20% ",
            "Vous aurez à payer ", (table[i].price - (table[i].price * "0.02")), " au lieu de ", table[i].price);
        }
    }
    return;
}
