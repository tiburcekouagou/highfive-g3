...
const firsDaytJuly = Date.parse("July 01, 2020");
const lastDayJuly = Date.parse("July 31, 2020");

function reduc() {
    for (let i = 0; i < orders.length; ++i) {
        const object = orders[i].timestamp * 1000;
        let priceValue = orders[i].price;

        if ((object > firsDaytJuly) && (object < lastDayJuly)) {
                priceValue = priceValue - priceValue * 20/100;
                console.log("Félicitation, vous avez bénéficié d'une réduction de 20%. Vous payez désormais :" + priceValue)
        } 
        else {
            console.log("Vous aurez une réduction, une prochaine fois")
        }
