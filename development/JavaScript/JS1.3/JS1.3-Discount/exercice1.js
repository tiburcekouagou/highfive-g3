...
const orders = [
  ...
  let first = String(Date.UTC(2020,06,01) / 1000) ;
  let last = String(Date.UTC(2020,06,30) / 1000) ;

  function julyReduction (objName){
    for (let i = 0; i < objName.length; i++){
      if(objName[i].timestamp >= first && objName[i].timestamp <= last){
        objName[i].price = Number(objName[i].price) - Number(objName[i].price) * 20 / 100;
        console.log("Le prix final apres application de la réduction sur la commande du client " + objName[i].id + " est de " + objName[i].price);
      }
    }
  }

julyReduction(orders);