...
function remise(example) {
   for (let i = 0 ; i < example.length; i++) {
       if (example[i].timestamp >= monthStartDate && example[i].timestamp <= monthEndDate) {
           result = example[i].price - (example[i].price * 0.2);
           console.log("le client " + example[i].id + " a obtenu une réduction de 20% et son prix est " + result.toFixed(2));
       };
   };
   return; 
};
