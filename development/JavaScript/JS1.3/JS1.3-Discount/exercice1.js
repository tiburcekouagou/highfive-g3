...
function reduct(prix) {
    prix = Number(prix) - (Number(prix) * 20) / 100;
    return String(prix);
}
for (let i = 0; i < 14; ++i) {
    if (orders[i].timestamp*1000 >= 1593558000000 && orders[i].timestamp*1000 <= 1596236400000) {
        let finalPrice = parseFloat(reduct(orders[i].price));
        console.log("Le client "+orders[i].id + " " + "ne devra payer que " + finalPrice + "€" +" au lieu de " + orders[i].price + "€");
    }
}
