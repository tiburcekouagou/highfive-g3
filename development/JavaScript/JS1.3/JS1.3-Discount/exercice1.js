...
function update(table) {
  ...
  for (i = 0; i < table.length; i++) {
    console.log((table[i].timestamp) >= minDate && (table[i].timestamp) <= maxDate);
    if ((table[i].timestamp) >= minDate && (table[i].timestamp) <= maxDate) {
      
      table[i].price = Number(table[i].price) - (Number(table[i].price) * 0.2);
      
      document.write(" Le client dont l'identité est " + table[i].id + " a obtenu une réduction de 20% sur son achat du mois de juillet 2020 et son prix actualisé est de  " + table[i].price + " " + "<br>")
    }
  }
  return;
}
