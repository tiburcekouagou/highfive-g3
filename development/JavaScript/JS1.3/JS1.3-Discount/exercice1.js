...
const monthStartDate = Date.UTC(2020, 06, 01) / 1000;
const monthEndDate = Date.UTC(2020, 06, 31) / 1000;
let result;
 

function reduction(example) {
    for (let a = 0 ; a < example.length; a++) {
        if (example[a].timestamp >= monthStartDate && example[a].timestamp <= monthEndDate) {
            result = example[a].price - (example[a].price * 0.2);
            console.log("le client " + example[a].id + " a obtenu une réduction de 20% et son prix est " + result.toFixed(2) + "€.");
        };
    };
    return; 
};
